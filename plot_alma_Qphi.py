...
def make_plot(observation_alma,maxobs_alma,pxsizeobs_alma,model_alma,maxmod_alma,pxsizemod_alma,
    ...
    ############################################################
    # Loading data
    hdu_obs=fits.open(observation_alma)
    hdu_mod=fits.open(model_alma)

    data_obs=(hdu_obs[0].data[0][0])*1000.0 # mJy/beam
    data_mod=hdu_mod[0].data # mJy/beam

    data_obs=data_obs/maxobs_alma
    data_mod=data_mod/maxmod_alma


    ############################################################
    # Determine the fov
    fov_obs=data_obs.shape[0]*pxsizeobs_alma
    fov_mod=data_mod.shape[0]*pxsizemod_alma
    ...
    cbar_1.set_label(r"Surface brightness (mJy/beam)",fontsize=lsize)
...
make_plot("/Users/users/bportilla/Documents/first_project/scripts/PDS70/observations/PDS70_cont-final.fits",max_obs,pxsize_obs,
          "../alma_model_rotated.fits",max_mod,pxsize_mod,
          beam_x,beam_y,-153.0)
